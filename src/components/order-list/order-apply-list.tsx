...
import { IOrders, Orders, OrdersApply } from '../../store/orders'
import { IconLoading } from '../icons'

import { OrderApplyListItem } from './order-apply-list-item'
...
@inject((stores: IStores, props: IOrderListProps) => ({
  ui: stores.ui,
  order: stores.ordersStore.types.get(OrdersApply.displayName),
}))
@observer
export class OrderApplyList extends React.Component<IOrderListProps> {
  public render() {
    ...
    return (
      <div className="order-list-container" style={{ height: ui!.getScreenHeight(0.88, 1) + 'px' }}>
        ...
        >
          {order.orders.map(o => (
            <OrderApplyListItem key={o.getId()} store={order as Orders} order={o} />
          ))}
